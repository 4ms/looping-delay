...
public:
	...
	void Write(std::span<T> output) {
		fwrite(output.data(), sizeof(T), output.size(), fp);
	}

	template<size_t size>
	void Write(std::array<T, size> &output) {
...
public:
	...
	bool Read(std::span<T> input) {
		return fread(input.begin().repr(), sizeof(T), input.size(), fp) == input.size();
	}

	template<class T, int size>
	bool Read(std::array<T, size> &input) {
