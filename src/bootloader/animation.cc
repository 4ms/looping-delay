...
void animate(Animation animation_type) {
	...
	switch (animation_type) {
		...
		case Animation::WAITING:
			// Flash Hold button when waiting
			step_time = 300 * TICKS_PER_MS;
			if (ctr == 0) {
				leds.set(Leds::Hold, true);
				leds.set(Leds::Ping, false);
				leds.set(Leds::Rev, false);
		...
		case Animation::RECEIVING:
			// Flash Ping button when receiving
			step_time = 250 * TICKS_PER_MS;
			if (ctr == 0) {
				leds.set(Leds::Ping, true);
				leds.set(Leds::Rev, false);
			} else if (ctr == 1) {
		...
		case Animation::SYNC:
			// Flash Ping button rapidly when syncing
		...
		case Animation::WRITING:
			// Flash Ping rapidly when writing
			step_time = 150 * TICKS_PER_MS;
			if (ctr == 0) {
				leds.set(Leds::Ping, false);
				leds.set(Leds::Rev, false);
			} else if (ctr == 1) {
